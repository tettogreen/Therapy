...
namespace UnityStandardAssets.ImageEffects
{
    ...
    class GlobalFog : PostEffectsBase
	{
		...
		[Tooltip("Fog will go away from startDistance to targetDistance")]
        ...
        void OnRenderImage (RenderTexture source, RenderTexture destination)
		{
			...
			if (delayTime <= Time.time && currentDistance != targetDistance) {
				currentDistance = Mathf.Lerp (currentDistance, targetDistance, 0.1f * Time.deltaTime) ;
			}
			...
			fogMaterial.SetVector ("_DistanceParams", new Vector4 (-Mathf.Max(currentDistance,0.0f), excludeDepth, 0, 0));
